...
from genesis_app.infrastructure.adapters.storage.sql_user_repository import SqlUserRepository
...
from genesis_app.infrastructure.repositories.sqlalchemy_rule_repository import (
...
class DIContainer:
    def __init__(self, probability_threshold: float = 0.3):
        ...
        if os.path.exists(physiological_ranges_path):
            ...
            logger.warning(
                f"Physiological ranges file not found: {physiological_ranges_path}. Validator disabled."
            )
            self.validator = None
    ...
    def reload_configuration(self) -> None:
        ...
        self.post_processor = PostProcessor(
            logic_loader=self.logic_loader, probability_threshold=self.post_processor.threshold
        )
