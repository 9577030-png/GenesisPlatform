from genesis_app.infrastructure.repositories.sqlalchemy_models import Base, RuleVersionModel
...
class SQLAlchemyRuleRepository(RuleRepository):
        ...
        with self.engine.connect() as conn:
            ...
            if "rule_versions" in inspector.get_table_names():
                ...
                if "tier" not in columns:
                    conn.execute(
                        "ALTER TABLE rule_versions ADD COLUMN tier VARCHAR(20) DEFAULT 'basic'"
                    )
                    conn.commit()
    ...
    def get_by_id(self, rule_id: str, version_id: int | None = None) -> RuleVersion | None:
        with self.Session() as session:
    ...
    def activate_version(self, rule_id: str, version_id: int) -> None:
        with self.Session() as session:
            ...
            session.query(RuleVersionModel).filter_by(version_id=version_id).update(
                {"is_active": True}
            )
            session.commit()
    ...
    def get_version_history(self, rule_id: str) -> list[RuleVersion]:
        with self.Session() as session:
            models = (
                ...
                .order_by(RuleVersionModel.created_at.desc())
                .all()
            )
            return [self._to_domain(m) for m in models]
