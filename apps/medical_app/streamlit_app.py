...
def analyze_data(payload, token):
    ...
    resp = requests.post(
        f"{API_BASE}/analyze_structured", json=payload, headers=headers, timeout=30
    )
    return resp
...
if not st.session_state.token:
    ...
    st.sidebar.markdown(
        f"вњ… Р’С‹ РІРѕС€Р»Рё РєР°Рє **{st.session_state.username}** (СЂРѕР»СЊ: {st.session_state.role})"
    )
...
page = st.sidebar.radio(
    "Р’С‹Р±РµСЂРёС‚Рµ СЂР°Р·РґРµР»", ["РђРЅР°Р»РёР·", "РђРґРјРёРЅРёСЃС‚СЂРёСЂРѕРІР°РЅРёРµ"]
)
...
if page == "РђРЅР°Р»РёР·":
    ...
    st.markdown(
        "Р’РІРµРґРёС‚Рµ РґР°РЅРЅС‹Рµ РїР°С†РёРµРЅС‚Р° Рё Р»Р°Р±РѕСЂР°С‚РѕСЂРЅС‹Рµ РїРѕРєР°Р·Р°С‚РµР»Рё РґР»СЏ РїРѕР»СѓС‡РµРЅРёСЏ РєР»РёРЅРёС‡РµСЃРєРѕРіРѕ Р·Р°РєР»СЋС‡РµРЅРёСЏ."
    )
    ...
    with st.sidebar:
        ...
        complaints = st.text_area(
            "Р–Р°Р»РѕР±С‹ (С‡РµСЂРµР· Р·Р°РїСЏС‚СѓСЋ)", value="fatigue, weakness"
        )
        medications = st.text_area(
            "РџСЂРёРЅРёРјР°РµРјС‹Рµ Р»РµРєР°СЂСЃС‚РІР° (С‡РµСЂРµР· Р·Р°РїСЏС‚СѓСЋ)", value=""
        )
        ...
        with col_val:
            param_value = st.number_input(
                "Р—РЅР°С‡РµРЅРёРµ", value=0.0, step=0.1, format="%.2f", key="param_value"
            )
        with col_unit:
            param_unit = st.text_input(
                "Р•РґРёРЅРёС†Р° (РѕРїС†РёРѕРЅР°Р»СЊРЅРѕ)",
                value="",
                placeholder="РЅР°РїСЂ. РјРјРѕР»СЊ/Р»",
                key="param_unit",
            )
        ...
        analyze_btn = st.button(
            "рџ”Ќ РђРЅР°Р»РёР·РёСЂРѕРІР°С‚СЊ", type="primary", use_container_width=True
        )

    # ---- РћСЃРЅРѕРІРЅР°СЏ РѕР±Р»Р°СЃС‚СЊ ----
    with st.expander(
        "рџ“ќ РЎС„РѕСЂРјРёСЂРѕРІР°РЅРЅС‹Р№ С‚РµРєСЃС‚ РґР»СЏ Р°РЅР°Р»РёР·Р°", expanded=False
    ):
    ...
    if analyze_btn:
        if not st.session_state.raw_text.strip():
            st.error(
                "РџРѕР¶Р°Р»СѓР№СЃС‚Р°, РґРѕР±Р°РІСЊС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРёРЅ Р»Р°Р±РѕСЂР°С‚РѕСЂРЅС‹Р№ РїР°СЂР°РјРµС‚СЂ."
            )
            st.stop()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                with st.expander(
                    "рџ“¦ РћС‚Р»Р°РґРѕС‡РЅР°СЏ РёРЅС„РѕСЂРјР°С†РёСЏ (Р·Р°РїСЂРѕСЃ Рё РѕС‚РІРµС‚)",
                    expanded=False,
                ):
                ...
                if grouped:
                    st.markdown(
                        "### рџ§¬ Р Р°СЃРїСЂРµРґРµР»РµРЅРёРµ РїРѕ СЃРёСЃС‚РµРјР°Рј РѕСЂРіР°РЅРѕРІ"
                    )
                ...
                if recommendations:
                    st.markdown(
                        "### рџ‘ЁвЂЌвљ•пёЏ Р РµРєРѕРјРµРЅРґР°С†РёРё РїРѕ СЃРїРµС†РёР°Р»СЊРЅРѕСЃС‚СЏРј"
                    )
                ...
                if insights:
                    st.markdown(
                        "### рџ§  РљР»РёРЅРёС‡РµСЃРєРёРµ РёРЅСЃР°Р№С‚С‹ (РїРѕРґСЂРѕР±РЅР°СЏ РёРЅС‚РµСЂРїСЂРµС‚Р°С†РёСЏ)"
                    )
                    for diag_id, insight in insights.items():
                        ...
                        with st.expander(f"**{label}** (ID: {diag_id})"):
                            ...
                            if differentials:
                                st.markdown(
                                    "#### рџ”Ќ Р”РёС„С„РµСЂРµРЅС†РёР°Р»СЊРЅР°СЏ РґРёР°РіРЅРѕСЃС‚РёРєР°"
                                )
                                for diff in differentials:
                                    st.markdown(
                                        f"- **РЈСЃР»РѕРІРёРµ:** `{diff.get('condition', '')}` в†’ {diff.get('text', '')}"
                                    )
                            ...
                            if red_flags:
                                ...
                                for rf in red_flags:
                                    st.markdown(
                                        f"- **{rf.get('condition', '')}** в†’ {rf.get('text', '')}"
                                    )
                            ...
                            if treatment_hints:
                                ...
                                for hint in treatment_hints:
                                    st.markdown(
                                        f"- **{hint.get('step', '')}** вЂ” {hint.get('note', '')}"
                                    )
                    ...
                    st.info(
                        "Р”Р»СЏ РІС‹СЏРІР»РµРЅРЅС‹С… РґРёР°РіРЅРѕР·РѕРІ РЅРµС‚ РґРѕРїРѕР»РЅРёС‚РµР»СЊРЅС‹С… РєР»РёРЅРёС‡РµСЃРєРёС… РёРЅСЃР°Р№С‚РѕРІ."
                    )
...
# =====================================================
    ...
    tab1, tab2, tab3 = st.tabs(
        ["рџ‘¤ РџРѕР»СЊР·РѕРІР°С‚РµР»Рё", "рџ“њ РСЃС‚РѕСЂРёСЏ", "вљ™пёЏ РџСЂР°РІРёР»Р°"]
    )
            ...
            if submitted:
                if new_username and new_password:
                    reg_resp = register_user(
                        st.session_state.token, new_username, new_password, new_role
                    )
    ...
    with tab3:
        ...
        config_path = st.text_input(
            "РџСѓС‚СЊ Рє С„Р°Р№Р»Сѓ (РѕС‚РЅРѕСЃРёС‚РµР»СЊРЅРѕ knowledge/)",
            value="configs/clinical_thresholds.yaml",
        )
        ...
        if "current_config_content" in st.session_state:
            new_content = st.text_area(
                ...
                value=st.session_state.current_config_content,
                height=400,
            )
            if st.button("РЎРѕС…СЂР°РЅРёС‚СЊ С„Р°Р№Р»"):
                save_resp = save_config_file(
                    st.session_state.token, st.session_state.current_config_path, new_content
                )
                if save_resp.status_code == 200:
                    st.success("Р¤Р°Р№Р» СЃРѕС…СЂР°РЅС‘РЅ")
                    st.session_state.current_config_content = new_content
                    if st.button(
                        "РџРµСЂРµР·Р°РіСЂСѓР·РёС‚СЊ РїСЂР°РІРёР»Р° РїРѕСЃР»Рµ СЃРѕС…СЂР°РЅРµРЅРёСЏ"
                    ):
                        reload_resp = reload_config(st.session_state.token)
                        if reload_resp.status_code == 200:
                            st.success("РљРѕРЅС„РёРіСѓСЂР°С†РёСЏ РїРµСЂРµР·Р°РіСЂСѓР¶РµРЅР°")
...
st.markdown("---")
st.caption(
    "РЎРёСЃС‚РµРјР° РёРЅС‚РµСЂРїСЂРµС‚Р°С†РёРё Р»Р°Р±РѕСЂР°С‚РѕСЂРЅС‹С… РґР°РЅРЅС‹С… v1.0 | API: localhost:8000"
)
