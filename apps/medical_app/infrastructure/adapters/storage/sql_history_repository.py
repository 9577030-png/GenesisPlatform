...
class SqlHistoryRepository(HistoryRepository):
    ...
    def _deserialize_report(self, json_str: str) -> AnalysisReport:
        ...
        for f_data in data["findings"]:
            ...
            try:
                risk = (
                    RiskLevel[risk_name] if risk_name in RiskLevel.__members__ else RiskLevel.NORMAL
                )
        ...
        for a_data in data["actions"]:
            ...
            try:
                urgency = (
                    Severity[urgency_name]
                    if urgency_name in Severity.__members__
        ...
        return AnalysisReport(
            findings=findings, actions=actions, explanation=data.get("explanation", "")
        )
    ...
    def save(self, patient_id: str, report: AnalysisReport) -> None:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO reports (patient_id, report_json) VALUES (?, ?)", (patient_id, json_data)
        )
        conn.commit()
        conn.close()
