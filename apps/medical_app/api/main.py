...
from medical_app.api.auth_config import authenticate_user, create_access_token, get_password_hash
...
try:
    ...
    logger.warning(
        f"РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ С€СЂРёС„С‚ DejaVuSans: {e}. Р‘СѓРґРµС‚ РёСЃРїРѕР»СЊР·РѕРІР°РЅ Helvetica."
    )
    FONT_NAME = "Helvetica"
...
from medical_app.api.routes import admin  # noqa: E402

admin.set_version_manager(
    container.version_manager
)  # РёРЅРёС†РёР°Р»РёР·РёСЂСѓРµРј РіР»РѕР±Р°Р»СЊРЅСѓСЋ РїРµСЂРµРјРµРЅРЅСѓСЋ РІ admin.py
...
class PatientRequest(BaseModel):
    ...
    gender: str
    age: int
    complaints: list[str] | None = Field(default_factory=list)
...
async def register(request: RegisterRequest, admin=Depends(require_admin)):
    ...
    return UserResponse(
        id=user.id, username=user.username, role=user.role, created_at=user.created_at.isoformat()
    )
...
async def list_users(admin=Depends(require_admin)):
    ...
    return [
        UserResponse(id=u.id, username=u.username, role=u.role, created_at=u.created_at.isoformat())
        for u in users
    ]
...
async def analyze_structured(request: AnalysisRequest, current_user=Depends(get_current_user)):
    try:
        gender = map_gender(request.patient.gender)
        patient = PatientProfile(
            id=request.patient.id,
        ...
        insights = interpreter.interpret(
            diagnoses=diagnoses_list, parameters=parameters, patient=patient
        )
...
@app.post("/export_pdf")
async def export_pdf(request: AnalysisRequest, current_user=Depends(get_current_user)):
    try:
        gender = map_gender(request.patient.gender)
        patient = PatientProfile(
            id=request.patient.id,
            ...
            for d in diagnoses:
                ...
                for rendered_line in lines:
                    if y < margin + 10 * mm:
                        ...
                        y = height - margin
                        c.setFont(FONT_NAME, 11)
                    c.drawString(margin, y, rendered_line)
                    y -= 5 * mm
                y -= 2 * mm
        ...
        if grouped:
            ...
            if y < margin + 15 * mm:
                ...
                y = height - margin
            c.drawString(
                margin, y, "Р Р°СЃРїСЂРµРґРµР»РµРЅРёРµ РїРѕ СЃРёСЃС‚РµРјР°Рј РѕСЂРіР°РЅРѕРІ:"
            )
            y -= 6 * mm
            c.setFont(FONT_NAME, 11)
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=report_{request.patient.id}.pdf"
            },
        )
