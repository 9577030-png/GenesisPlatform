...
class AnalysisPipeline:
    ...
    def _get_rules_version(self) -> str:
        if self.rule_repo:
            ...
            version_str = ",".join(
                f"{r.rule_id}:{r.version_id}" for r in sorted(active, key=lambda x: x.rule_id)
            )
            import hashlib
    ...
    def run_structured(
        self, patient: PatientProfile, raw_text: str, user_id: int = None
    ) -> AnalysisReport:
                ...
                try:
                    ...
                    if cached_result:
                        ...
                        report = AnalysisReport(
                            findings=[
                                ClinicalFinding(**f) for f in cached_result.get("findings", [])
                            ],
