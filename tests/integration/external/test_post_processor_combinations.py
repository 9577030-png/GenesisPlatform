...
def test_combination_primary_hyperparathyroidism():
    ...
    findings = [
        ClinicalFinding(
            id="hypercalcemia", title="Hypercalcemia", probability=0.8, risk=RiskLevel.HIGH
        ),
        ClinicalFinding(
    ...
    combined = [
        ...
        if d.get("combined") and d["id"] == "primary_hyperparathyroidism"
    ]
...
def test_combination_hepatorenal_syndrome():
    ...
    findings = [
        ClinicalFinding(
            id="acute_kidney_injury", title="AKI", probability=0.6, risk=RiskLevel.HIGH
        ),
        ClinicalFinding(
            id="acute_hepatitis", title="Acute hepatitis", probability=0.6, risk=RiskLevel.HIGH
        ),
    ]
    report = AnalysisReport(findings=findings, actions=[], explanation="")
    result = container.post_processor.process(report)
    combined = [
        d for d in result["diagnoses"] if d.get("combined") and d["id"] == "hepatorenal_syndrome"
    ]
    assert len(combined) == 1
...
def test_combination_septic_syndrome():
    ...
    result = container.post_processor.process(report)
    combined = [
        d for d in result["diagnoses"] if d.get("combined") and d["id"] == "septic_syndrome"
    ]
    assert len(combined) == 0, (
        "Septic syndrome combination should not appear because systemic_inflammation is excluded"
    )
...
def test_combination_metabolic_bone_disease():
    ...
    result = container.post_processor.process(report)
    combined = [
        d for d in result["diagnoses"] if d.get("combined") and d["id"] == "metabolic_bone_disease"
    ]
    assert len(combined) == 1
