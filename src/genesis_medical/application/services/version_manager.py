...
from genesis_medical.domain.exceptions import VersionNotFoundError
from genesis_medical.domain.rule_version import RuleTier, RuleVersion
...
class VersionManager:
    ...
    def load_from_yaml(
        self, rule_id: str, yaml_path: Path, created_by: str = "system"
    ) -> RuleVersion:
        ...
        tier = (
            RuleTier.ENRICHED
            if self.interpretation_mapper.is_enriched(base_name)
            else RuleTier.BASIC
        )
    ...
    def hot_reload(self, created_by: str = "system") -> list[RuleVersion]:
        ...
        for yaml_file in self.config_dir.rglob("*.yaml"):
            ...
            rule_id = (
                str(yaml_file.relative_to(self.config_dir)).replace("\\", "/").replace(".yaml", "")
            )
            try:
                version = self.load_from_yaml(rule_id, yaml_file, created_by)
                new_versions.append(version)
                logger.info(
                    f"Loaded {rule_id} (version {version.version_id}, tier={version.tier.value})"
                )
