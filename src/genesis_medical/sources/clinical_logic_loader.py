...
class ClinicalLogicLoader:
    ...
    def get_diagnosis_labels(self) -> dict[str, str]:
        return cast(dict[str, str], self.get_config().get("diagnosis_labels", {}))

    def get_system_groups(self) -> dict[str, list[str]]:
        return cast(dict[str, list[str]], self.get_config().get("system_groups", {}))

    def get_allowed_primary(self) -> list[str]:
        return cast(list[str], self.get_config().get("allowed_primary", []))
