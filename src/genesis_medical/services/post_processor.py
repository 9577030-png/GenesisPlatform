...
class PostProcessor:
    def __init__(
        ...
        logger.info(
            f"Loaded {len(self.diagnosis_labels)} labels, {len(self.system_groups)} system groups, {len(self.allowed_primary)} primary diagnoses"
        )
    ...
    def process(self, report: AnalysisReport) -> dict[str, Any]:
        ...
        for action in all_actions:
            ...
            recommendations_by_specialty[spec].append(
                {"urgency": action.urgency.value, "tests": action.additional_tests}
            )

        # 8. РћР±С‰РёР№ СѓСЂРѕРІРµРЅСЊ СЂРёСЃРєР°
        max_risk = max(
            (f.risk for f in significant),
            key=lambda r: r.value if hasattr(r, "value") else 0,
            default=None,
        )
        overall_risk_level = (
            max_risk.label if max_risk and hasattr(max_risk, "label") else "РќРѕСЂРјР°"
        )

        # 9. Р—Р°РєР»СЋС‡РµРЅРёРµ
        conclusion = self._build_conclusion(
            diagnoses, grouped, recommendations_by_specialty, max_risk
        )
    ...
    def _apply_combinations(self, findings: list[ClinicalFinding]) -> tuple:
        ...
        for combo in self.combinations:
            ...
            combined_diagnoses.append(
                {
                    ...
                    "description": combo.get("recommendation")
                    or combo.get("description")
                    or "РљРѕРјР±РёРЅРёСЂРѕРІР°РЅРЅРѕРµ СЃРѕСЃС‚РѕСЏРЅРёРµ",
                }
            )
            ...
            try:
                ...
                urgency = (
                    Severity[urgency_str]
                    if urgency_str in Severity.__members__
    ...
    def _create_actions_from_combinations(
    ...
    def _build_conclusion(self, diagnoses, grouped, recommendations_by_specialty, max_risk) -> str:
        ...
        if recommendations_by_specialty:
            for spec, recs in recommendations_by_specialty.items():
                ...
                for r in recs:
                    lines.append(
                        f"    - РЎСЂРѕС‡РЅРѕСЃС‚СЊ: {r['urgency']}, С‚РµСЃС‚С‹: {', '.join(r['tests'])}"
                    )
        else:
